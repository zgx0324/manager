...
<head>
    ...
    <script type="text/javascript">
        ...
        $(function(){
            $("[name='ctname']").change(function(){
                var ctid=$(this).next().val();
                if(this.value.length<6&&this.value.length>0){
                    $.ajax({
                    url:"${pageContext.request.contextPath}/categoryThird.do/checkCtname",
                    data:{
                        ctname:encodeURI(this.value)
                    },
                    success:function(data){
                        if(data == "ok"){
                            $("#sbm"+ctid).attr("type","submit");
                        }else{
                            alert("类目名已存在不可使用");
                            $("#sbm"+ctid).attr("type","button");
                        }
                    }
                });
                }else{
                    alert("只可输入六个字符以内的类目名");
                    $("#sbm"+ctid).attr("type","button");
                }
            });
        });
        ...
        $(function () {
            $("[name='add']").click(function () {
                var ctid=$(this).val();
                var state=$("#state"+ctid).val();
                var ctname=$("#ctname"+ctid).val();
                if($(this).attr("type")=="submit"&&state!=""){
                    $.ajax({
                        url:"${pageContext.request.contextPath}/categoryThird.do/add",
                        data:{
        ...
        $(function () {
            $("[name='update']").click(function () {
                ...
                if($(this).attr("type")=="submit"&&state!=""){
                    $.ajax({
                        url:"${pageContext.request.contextPath}/categoryThird.do/update",
                        data:{
                            csid:"${categorySecond.csid}",
                            ctid:ctid,
                            state:state,
        ...
        $(function(){
            $("[name='del']").click(function(){
                if(confirm("确认删除该类目吗！")){
                    ...
                    $.ajax({
                        url:"${pageContext.request.contextPath}/categoryThird.do/del?ctid="+this.value,
                        success:function(data){
                            if(data == "true"){
            ...
            <div class="container-fluid">
                <div class="row">
                    <div class="col-md-6" style="width: 800px;">
                        <div class="card ">
                            <div class="header">
                                <h4 class="title">类别管理(三级类目)</h4>
                                <p class="category">Backend development</p>
                                <h5 class="title">${categorySecond.csname}（二级类目）</h5>
                            </div>
                            ...
                            <div class="content">
                                <c:if test="${empty categoryThirdList}">
                                ...
                                <c:if test="${!empty categoryThirdList}">
                                    ...
                                    <table class="table" id="table">
                                        ...
                                        <c:forEach items="${categoryThirdList}" var="categoryThird">
                                                <tr id="tr${categoryThird.ctid}">

                                                    <td><input type="text" class="form-control" readonly=true name="ctid" value="${categoryThird.ctid}">
                                                    </td>
                                                    <td><input type="text" class="form-control" id="ctname${categoryThird.ctid}"
                                                               name="ctname" placeholder="类别名称"
                                                               value="${categoryThird.ctname}" onfocus="this.value = '';"
                                                               onblur="if (this.value == '') {this.value = '${categoryThird.ctname}';}">
                                                        <input type="hidden" name="getctid" value="${categoryThird.ctid}">
                                                    </td>
                                                    <td>
                                                        <div style="width: 130px;">
                                                            <input type="number" class="form-control"
                                                                   placeholder="类别优先级" name="state"
                                                                   id="state${categoryThird.ctid}"
                                                                   value="${categoryThird.state}">
                                                        </div>
                                                    </td>
                                                    <td class="td-actions text-right">
                                                        <button type="submit" name="update" rel="tooltip"
                                                                value="${categoryThird.ctid}" id="sbm${categoryThird.ctid}"
                                                                title="修改" class="btn btn-info btn-simple btn-xs">
                                                            <i class="fa fa-edit"></i>

                                                        </button>
                                                        <button type="button" rel="tooltip" title="删除"
                                                                name="del" value="${categoryThird.ctid}"
                                    ...
                                    <hr>
                                        <div class="stats">
                                            <i class="fa fa-history"></i>添加类别
                                                <table class="table">
                                                    <tr>
                                                        <td><input type="text" class="form-control" id="ctid0"
                                                                   readonly=true name="ctid" placeholder="id自动生成"></td>
                                                        <td><input type="text" class="form-control" name="ctname" id="ctname0" placeholder="类目名称">
